...
2. **User-level conventions.** Append a "Product Source of Truth (sapho)" section to your
   ...
   (f) when a question spans several products (overview, portfolio, prioritization,
   what's stale), read ALL truth files and offer a synthesized "atlas" view — a dashboard
   artifact if the platform can render one, structured text otherwise. Always generated
   fresh; never hosted or cached.
